...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, clip=False, classname=None, device='cuda'):
    ...
    def forward(self, x):
        ...
        out = self.linear(out)
        if self.clip:
          self.class_vectors = self.class_vectors.to(x.device)
...
def ResNet18(num_classes=10, clip=False, classname=None):
